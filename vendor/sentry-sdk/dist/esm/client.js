...
import { PTZ_KEEPALIVE_MS, RECORDING_MAX_CLIP_SEC, RECORDING_MAX_SLICE_SEC, } from "./models.js";
import { parseIceCandidates, parsePtzResult, parseSessionStatus, parseTowerDetail, parseTowerInfo, parseTowerList, parseViewerSession, } from "./validate.js";
function refId(ref) {
...
export class SentryClient {
    ...
    async fetchRecordingSlice(ref, start, durationSec, opts = {}) {
        ...
        if (durationSec > RECORDING_MAX_SLICE_SEC) {
            ...
            throw new RangeError(`durationSec ${durationSec} exceeds the ${RECORDING_MAX_SLICE_SEC}s cap`);
        }
        const id = encodeURIComponent(refId(ref));
    ...
    async fetchClip(ref, start, durationSec, opts = {}) {
        ...
        if (durationSec > RECORDING_MAX_CLIP_SEC) {
            throw new RangeError(`durationSec ${durationSec} exceeds the ${RECORDING_MAX_CLIP_SEC}s clip cap`);
        }
        const id = encodeURIComponent(refId(ref));
        const q = `start=${encodeURIComponent(start)}&duration=${encodeURIComponent(String(durationSec))}`;
        const { body } = await this.http.send({
 ...
 * remember.
 */
export function clampMove(params) {
