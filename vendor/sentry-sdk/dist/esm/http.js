 ...
 */
import { NetworkError, RequestAbortedError, RequestTimeoutError, SentryError, errorFromEnvelope, } from "./errors.js";
export class HttpTransport {
    ...
    async send(args) {
        ...
        try {
            ...
            if (timedOut()) {
                throw new RequestTimeoutError(`${args.method} ${args.path} timed out after ${timeoutMs}ms`, { timeoutMs, path: args.path, cause });
            }
        ...
        if (args.accept === "blob" && res.ok) {
            if (typeof res.arrayBuffer !== "function") {
                throw new SentryError(`${args.method} ${args.path} needs a fetch whose Response supports arrayBuffer()`, { code: "response_invalid", status: res.status, path: args.path });
            }
        ...
        try {
            ...
            throw new SentryError(`${args.method} ${args.path} returned ${res.status} with unparseable JSON`, { code: "response_invalid", status: res.status, path: args.path, cause });
        }
    }
    /**
    ...
    toError(status, text, contentType, path) {
        if (text && contentType.includes("json")) {
            try {
                ...
                if (err && typeof err.code === "string") {
                    return errorFromEnvelope({ code: err.code, message: typeof err.message === "string" ? err.message : err.code }, { status, path });
                }
            }
            catch {
        ...
        return errorFromEnvelope({ code: syntheticCode(status), message: snippet || `HTTP ${status}` }, { status, path });
    }
}
/* ------------------------------------------------------------------ *
 * Helpers
