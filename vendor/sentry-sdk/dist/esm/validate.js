...
class Check {
    ...
    /** Throw once, with every issue found, or return the value. */
        ...
        if (this.issues.length > 0) {
            throw new ValidationError(`${what} from the coordination service did not match the session-protocol shape`, { issues: this.issues, status: ctx.status, path: ctx.path });
        }
        return value;
    }
}
 ...
 */
export function assertNoProjectionLeak(raw, ctx = {}) {
    const leaked = [];
    const walk = (node, at) => {
    ...
    walk(raw, "");
    if (leaked.length > 0) {
        throw new ProjectionLeakError(`projection leaked ${leaked.length} field(s) §A.6 says MUST NOT reach a viewer: ` +
            leaked.join(", ") +
 ...
 */
export function parseViewerSession(raw, ctx = {}) {
    const c = new Check();
    const o = c.obj("$", raw) ?? {};
    const value = {
...
export function parseSessionStatus(raw, ctx = {}) {
    const c = new Check();
    const o = c.obj("$", raw) ?? {};
    const value = {
        session_id: c.str("session_id", o["session_id"]),
...
export function parseIceCandidates(raw, ctx = {}) {
    const c = new Check();
    const o = c.obj("$", raw) ?? {};
    const value = {
        candidates: c
 ...
 */
export function parsePtzResult(raw, ctx = {}) {
    const c = new Check();
    const o = c.obj("$", raw) ?? {};
    const value = {
        session_id: c.str("session_id", o["session_id"]),
 ...
 */
export function parseCameraInfo(c, path, raw) {
    const o = c.obj(path, raw);
    if (!o)
 ...
 */
export function parseTowerHealth(c, path, raw) {
    const o = c.obj(path, raw);
    if (!o)
        return {};
    const out = {};
 ...
 */
export function parseTowerInfo(raw, ctx = {}) {
...
export function parseTowerList(raw, ctx = {}) {
    assertNoProjectionLeak(raw, ctx);
    const c = new Check();
    const o = c.obj("$", raw) ?? {};
    const value = {
 ...
 */
export function parseTowerDetail(raw, ctx = {}) {
    assertNoProjectionLeak(raw, ctx);
    const c = new Check();
