...
/* ------------------------------------------------------------------ *
 * §6.1 — grant vocabulary (the viewer sees the words, never the grant)
 * ------------------------------------------------------------------ */
...
export type PtzCommand =
  | { camera: CameraIndex; action: "move"; params: PtzMoveParams }
  | { camera: CameraIndex; action: "stop"; params?: PtzStopParams }
  | { camera: CameraIndex; action: "keepalive"; params?: Record<string, never> }
  | { camera: CameraIndex; action: "status"; params?: Record<string, never> }
  | { camera: CameraIndex; action: "set_home"; params?: Record<string, never> };
...
export interface PtzSetHomeResult {
  ...
  home?: { pan: number; tilt: number; zoom: number };
...
/* ------------------------------------------------------------------ *
 * §3.1 — tower / camera inventory, viewer-relevant fields only
 * ------------------------------------------------------------------ */
...
/* ------------------------------------------------------------------ *
 * §3.1 / §3.4 — health as surfaced to a viewer
 * ------------------------------------------------------------------ */
...
export type ApiErrorCode =
  ...
  | "rate_limited"
  | "internal"
  // §7.1 — "Viewer-facing HTTPS adds:"
  | "tower_offline"
  | "tower_timeout"
  | "not_authorized"
  // §4.2.3 — covers both "no such tower" and "not yours", deliberately
  // indistinguishable so the inventory endpoint cannot enumerate the fleet.
  | "tower_unknown"
  // Rename route (`PATCH /v1/viewer/towers/{device_id}`) — the server refused
  // the label itself: empty, over-long, or carrying control characters.
  | "invalid_label"
  | (string & {});
...
/* ------------------------------------------------------------------ *
 * DEFERRED — recording / playback (§9, architecture §17)
 * ------------------------------------------------------------------ */
